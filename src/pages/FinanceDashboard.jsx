...
export default function FinanceDashboard() {
  ...
  useEffect(() => { try { localStorage.setItem("fin_txns", JSON.stringify(transactions)); } catch { } }, [transactions]);
    ...
    <div className="min-h-screen bg-[#080D1A] text-slate-100">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between px-4 sm:px-6 h-auto md:h-16 py-3 md:py-0 border-b border-white/10 backdrop-blur bg-[#080D1A]/90 sticky top-0 z-50 gap-3">
        <div className="flex items-center gap-3">
          <div style={{ width: 32, height: 32, background: "linear-gradient(135deg, #F59E0B, #F97316)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>₹</div>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, letterSpacing: "-0.01em" }}>FinFlow</span>
        </div>

        <div className="flex flex-wrap gap-2 bg-white/5 rounded-xl p-1 w-full md:w-auto justify-center">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: "6px 18px", borderRadius: 8, border: "none", background: activeTab === tab ? "rgba(255,255,255,0.1)" : "transparent", color: activeTab === tab ? "#F1F5F9" : "#64748B", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, textTransform: "capitalize", transition: "all 0.15s" }}>
              {tab}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 justify-between md:justify-end w-full md:w-auto">
          ...
          {isAdmin && (
            <button onClick={() => setShowModal(true)} style={{ padding: "8px 18px", borderRadius: 10, border: "none", background: "linear-gradient(135deg, #F59E0B, #F97316)", color: "#0A0F1E", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
              + Add
            </button>
          )}
        </div>
        ...
        {activeTab === "overview" && (
          <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 28 }}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <SummaryCard label="Net Balance" value={fmt(balance)} icon="⚖️" color="#F59E0B" sub="Across all time" trend={0} />
              <SummaryCard label="Total Income" value={fmt(totalIncome)} icon="↑" color="#10B981" sub={`+${incomeGrowth}% vs last month`} trend={1} />
              <SummaryCard label="Total Expenses" value={fmt(totalExpenses)} icon="↓" color="#EF4444" sub="This period" trend={-1} />
            </div>
  ...
  <ResponsiveContainer width="100%" height={220}>
    <AreaChart data={MONTHLY_TREND}>
                ...
                <ResponsiveContainer width="100%" height={150}>
                  <PieChart>
                    <Pie data={categoryBreakdown} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={45} outerRadius={72} strokeWidth={0}>
                      {categoryBreakdown.map((e, i) => <Cell key={i} fill={CATEGORY_COLORS[e.name] || "#64748B"} />)}
                    </Pie>
                    <Tooltip content={<CustomTooltip />} />
                  </PieChart>
                </ResponsiveContainer>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <h3 style={{ margin: "0 0 4px", fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 600 }}>Monthly Income vs Expenses</h3>
        ...
        {activeTab === "transactions" && (
          <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-3 items-stretch sm:items-center">
              ...
              {[
                { label: "Type", val: filterType, set: setFilterType, opts: [["all", "All Types"], ["income", "Income"], ["expense", "Expense"]] },
                { label: "Category", val: filterCategory, set: setFilterCategory, opts: [["all", "All Categories"], ...ALL_CATEGORIES.map(c => [c, c])] },
                { label: "Sort", val: sortBy, set: setSortBy, opts: [["date-desc", "Newest First"], ["date-asc", "Oldest First"], ["amount-desc", "Highest Amount"], ["amount-asc", "Lowest Amount"]] },
              ].map(({ val, set, opts }) => (
                <select key={opts[0][0]} value={val} onChange={e => set(e.target.value)}
              ...
              {isAdmin && (
                <button onClick={exportCSV} style={{ padding: "10px 16px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "#94A3B8", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, whiteSpace: "nowrap" }}>
                  ↓ Export CSV
                </button>
              )}
            </div>
    ...
    filtered.map((t, i) => (
      ...
      >
        ...
        {activeTab === "insights" && (
          <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {/* Insight Cards */}
            <div style={{ display: "grid", gap: 16 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                {
                ...
                <div key={card.title} style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${card.color}30`, borderRadius: 16, padding: "24px 28px", position: "relative", overflow: "hidden" }}>
                  ...
                  return (
                    <div key={c.name}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 6 }}>
                        ...
                        <span style={{ color: "#E2E8F0", fontWeight: 600 }}>{fmt(c.value)} <span style={{ color: "#64748B", fontWeight: 400 }}>({pct}%)</span></span>
                      </div>
